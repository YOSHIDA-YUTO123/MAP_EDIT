...
// モーションのロード処理
//===================================================
unique_ptr<CMotion> CMotion::Load(const char* pLoadFileName, vector<CModel*>& pModel, int* pOutModel,const int nNumMotion, LOAD type)
{
	// モーションを生成
	unique_ptr<CMotion> pMotion = make_unique<CMotion>();
...
bool CMotion::IsEventFrame(const int nType)
{
	...
	{
		int Start = m_aInfo[nType].aStartFrame[nCnt];
		int End = m_aInfo[nType].aEndFrame[nCnt];
...
{
	// イベントフレームの総数分回す
	int Start = m_aInfo[nType].aStartFrame[nIdx];
	int End = m_aInfo[nType].aEndFrame[nIdx];
...
void CMotion::UpdateBlendMotion(CModel** pModel, int nIdx)
{
	...
	// ブレンドフレームを計算
	float fBlendFrame = (float)m_aInfo[m_nTypeBlend].aKeyInfo[m_nKeyBlend].nFrame;
...
void CMotion::Update(CModel** pModel,const int nNumModel)
{
	...
	{
		// ブレンドのフレームを計算
		int nBlendFrame = m_aInfo[m_nType].aKeyInfo[m_nNumKey - 1].nFrame;
	...
	// フレームを計算
	int nFrame = m_aInfo[m_nType].aKeyInfo[m_nKey].nFrame;
	...
	// ブレンドフレームを計算
	nFrame = m_aInfo[m_nTypeBlend].aKeyInfo[m_nKeyBlend].nFrame;
...
unique_ptr<CLoderText> CLoderText::LoadTextFile(const char* pFileName, vector<CMotion::Info>& Info, std::vector<CModel*>& pModel, int* OutNumModel,const int nNumMotion)
{
	...
	if (File.is_open() == true)
	{
		...
		while (getline(File, line))
		{
			...
			// モデルのロード処理
			if (pLoder->LoadModel(pModel, nNumModel, input, line))
			{
				nNumModel++;
			}
			...
			{
				bCharacterSet = pLoder->LoadCharacterSet(pModel, line, input);
			}
...
// モデルのロード処理
//===================================================
bool CLoderText::LoadModel(std::vector<CModel*>& pModel, int nCnt, string input, string line)
{
	int nNumParts = 0;
...
// キャラクターのロード処理
//===================================================
bool CLoderText::LoadCharacterSet(std::vector<CModel*>& pModel, string line, string input)
{
	static int nIdx = 0;
	int nNumParts = 0;
	int nParent = 0;
